...
statAnalysis <- function(file, Frule = 0.8, normM = "NONE", imputeM = "KNN", glog = TRUE, FDR = TRUE, 
    ntree = 500, nvarRF = 5, scaling = "Pareto", silt = 20, pcax = 1, pcay = 2, Labels = TRUE, upper.lim = 2, 
    ...
    if (glog) {
        ...
        log = sT_univariate(logfile, FDR = FDR, upper.lim = upper.lim, lower.lim = lower.lim, sig.lim = sig.lim)
    } else {
        setwd("./statTarget/statAnalysis/")
        ...
        cat("\n")
        cat("* Step 7: Univariate Test Start...! Time: ", date(), "\n")
        log = sT_univariate(normfile, FDR = FDR, upper.lim = upper.lim, lower.lim = lower.lim, sig.lim = sig.lim)
    }
    cat("\n", "Output Link:", getwd(), "\n")
