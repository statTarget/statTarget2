...
vignette: >
  %\VignetteIndexEntry{statTarget 2.0}
...
experiments of metabolomics and proteomics. statTarget is a streamlined tool 
...
A streamlined tool provides graphical user interface for quality control based signal correction, integration of metabolomics and proteomics data from multi-batch experiments, and the comprehensive statistic analysis in non-targeted or targeted metabolomics and proteomics. 
(URL: https://stattarget.github.io)
 

## What does statTarget offer statistically

The statTarget 2.0 has two basic sections. The first section 
...
section is Statistical Analysis (See the statAnalysis function). It provides comprehensively computational and statistical methods that are commonly applied to analyze metabolomics data,
...
`Section 1 - Signal Correction` (the shiftCor function) provide 'machine learning method' for QC based signal correction. i.e. QC-RFSC and  QCRLSC that fit the QC data, 
and each metabolites in the true sample will be normalized to the QC sample.
To avoid overfitting of the observed data, generalised 
cross-validation (GCV) would be automatically applied for QC-RLSC, 
when the QCspan was set at 0. 

`Section 2 - Statistical Analysis ` (the statAnalysis function) provide features including Data 
preprocessing, Data descriptions, Multivariate statistics analysis and 
Univariate analysis. 


Data preprocessing : 80-precent rule, sum normalization (SUM) and probabilistic 
quotient normalization (PQN), glog transformation, KNN imputation, 
median imputation, and minimum imputation.
...
Multivariate statistics analysis : PCA, PLSDA, VIP, Random forest, Permutation importance.
...
```
<img src=
"https://github.com/13479776/Picture/raw/master/interface_statTarget.jpg" 
...
Download the [Reference manual](https://github.com/13479776/Picture/raw/master/work%20flow.pptx) and [example data](https://github.com/13479776/Picture/raw/master/Data_example.zip) .
...
`Meta File`
  ...
  (c) Batch: The analysis blocks or batches
  (d) Sample name should be consistent in Meta file and Profile file
...
"minHalf"; median values, "median". (Default: KNN) 
 ...
 The machine learning method for QC based signal correction. i.e.QC-RFSC
 and QC-RLSC.
...
The smoothing parameter for QC-RLSC which controls the bias-variance tradeoff. 
...
The number of trees to grow in random forest model.
...
`Permutation times or silt` 


The number of random permutation times for PLS-DA model and feature importance in randomForest (Default: 20) 
...
See ?statAnalysis for off-line help
```


## Generation of input file (the transX function)

The transX function is to generate statTarget input file formats from Mass Spectrometry Data softwares, such as XCMS, SIEVE and SKYLINE. '?transX' for off-line help.
...
Download the [example data](https://github.com/13479776/Picture/raw/master/Data_example.zip) .
...
-- After_shiftCor             # The fold for intergrated and corrected data 
   ...
   -- loplot                    # The fold for signal correction figures
      -- *.pdf                  # The signal correction figures for each features
...
Loplot (left): the visible figure of signal correction for each features
