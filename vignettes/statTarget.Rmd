...
# For mac PC,  the GUI function 'statTargetGUI()' need the XQuartz instead of X11 support. Download it from https://www.xquartz.org. 
# R version > 3.6.0 are recommended for statTargetGUI function. The R core team have updated the R > 3.6.0 that support the Mac OS systerm based RGtk2.
...
`Meta File`
  ...
  (4) Sample: Sample name should be consistent in Meta file and Profile file. *The QC sample name should be taged with "QC". 
